...
public class JsonUtil<T> {
	private static final String DevUtil_JSON_ERROR = "com.imcore.common.util.JsonError";
	...
	public static String getJsonValueByKey(String json, String key) {
		...
		try {
			...
			DevUtil.e(DevUtil_JSON_ERROR, e.getLocalizedMessage());
		}
		return value;
	}
	 ...
	 */
	public static <T> ArrayList<T> toObjectList(String json, Class<T> cls) {
	...
	public static List<String> toJsonStrList(String json) {
		...
		try {
			...
			DevUtil.e(DevUtil_JSON_ERROR, e.getMessage());
		}
		return strList;
	}
	 ...
	 * 获得服务端数据， 解析json数组
	 * 
	 * @param getObejectByKeyFromResponse
	 * @param cls
	 * @param jsonArrayKey
	 * @return
	 * @throws JSONException
	 ...
	 * 
	 */
	public static <T> ArrayList<T> getObjectList(
			JSONObject getObejectByKeyFromResponse, Class<T> cls,
			String jsonArrayKey) throws JSONException {
		ArrayList<T> listEntify = null;
		JSONArray jsonArray = getObejectByKeyFromResponse
				.getJSONArray(jsonArrayKey);
		for (int i = 0; i < jsonArray.length(); i++) {
			listEntify = (ArrayList<T>) JsonUtil.toObjectList(
					jsonArray.toString(), cls);
		}

		return (ArrayList<T>) listEntify;

	}
/**
 * "data":[{}]
 * @param getJsonArrayByKeyFromResponse
 * @param cls
 * @return
 * @throws JSONException
 */
	public static <T> ArrayList<T> getObjectListFromJsonArray(
			JSONArray getJsonArrayByKeyFromResponse, Class<T> cls)
			throws JSONException {
		ArrayList<T> listEntify = null;
		for (int i = 0; i < getJsonArrayByKeyFromResponse.length(); i++) {
			listEntify = (ArrayList<T>) JsonUtil.toObjectList(
					getJsonArrayByKeyFromResponse.toString(), cls);
		}
		return (ArrayList<T>) listEntify;
	 ...
	 * 
	 */
	public static <T> T getObject(JSONObject getObejectByKeyFromResponse,
			Class<T> cls, String jsonObjectKey) throws JSONException {
		T obj = null;
		JSONObject object = getObejectByKeyFromResponse
				.getJSONObject(jsonObjectKey);
		obj = JsonUtil.toObject(object.toString(), cls);
		return obj;
